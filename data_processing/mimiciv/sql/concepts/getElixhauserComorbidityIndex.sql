-- Description: This query creates a new getelixhausercomorbidityindex table
--           that contains the Elixhauser Comorbidity Index for each patient.
-- Source: https://github.com/arnepeine/ventai/blob/main/getElixhauser_score.sql
-- Execution time: Roughly 1 minute.
-- Number of Rows: 431088 (431 thousand)

SET search_path TO public, mimiciv_derived, mimiciv_core, mimiciv_hosp, mimiciv_icu, mimiciv_ed;

DROP TABLE IF EXISTS getelixhausercomorbidityindex; CREATE TABLE getelixhausercomorbidityindex AS

with icd as
(
  select subject_id, hadm_id, seq_num, icd_code AS icd9_code -- Note, MIMIC-IV includes both ICD-9 and ICD-10 codes,
															 -- we only use ICD-9 to enable VentAI code compatility.
  from diagnoses_icd
  where seq_num != 1 -- we do not include the primary icd-9 code
  AND icd_version = 9 -- we only include ICD-9 codes
)
, eliflg as
(

...
--      which discretizes the ICU stays into a grid using a predefined 1-hour window.
--      The table contains the average or maximum values of the measurements for each hour.
...
grid as
(
	SELECT stay_id, subject_id, hadm_id, generate_series(mint, maxt, interval '1 hours') as start_time
	FROM minmax
...
SELECT ot.stay_id, ot.subject_id, ot.hadm_id, start_time
	...
	, avg(PaO2FiO2ratio) as pao2fio2ratio, avg(BANDS) as BANDS
	, avg(CRP) as CRP
	-- others
...
LEFT JOIN merged_overalltable ot
	...
	AND ot.charttime <  g.start_time + '1 hours'
